...
if(isset($_COOKIE['user'])) {
	...
	while($userinfo= mysqli_fetch_array($user)) {
		if($password != $userinfo['password']) {
			...
			$id=$_GET["id"];
			$userid = $userinfo['id'];
			$sql="DELETE FROM concept WHERE userid = '$userid' AND id='$id'";
			$result=mysqli_query($db_link, $sql);
			if (!$result) error(mysqli_error());
			header("location: concepts.php#concepts");
		}
	}
